...
int main()
{
    ...
    rewind(fpTextVariable);
    while(fgets(line,sizeof(line),fpTextVariable))
    {
        if(!(aux = strchr(line,'\n')))
        {
            fprintf(stderr,"Error - No se pudo procesar archivo %s", FILE_TEXT_FIXED);
        ...
        *aux = '\0';
        ...
        /** promedio alumno **/
        aux = strrchr(line,'|');
        sscanf(aux + 1,"%f",&reg.average);
        *aux = '\0';
        /** apellido alumno **/
        aux = strrchr(line,'|');
        sscanf(aux + 1,"%s",&reg.surName);
        *aux = '\0';
        /** nombre alumno **/
        aux = strrchr(line,'|');
        sscanf(aux + 1,"%s",&reg.name);
        *aux = '\0';
        /** dni alumno **/
        sscanf(line,"%ld",&reg.dni);
        *aux = '\0';

        fwrite(line,1,sizeof(line),fpDataVariable);
    }
